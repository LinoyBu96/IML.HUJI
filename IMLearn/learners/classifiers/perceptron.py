...
class Perceptron(BaseEstimator):
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        if not self.fitted_:
            raise ValueError("Estimator must first be fitted before calling this function")
        if X.ndim == 1:
            if X.shape[0] == self.coefs_.shape[0] - 1:
                X = np.append(0, X)
        elif X.shape[1] == self.coefs_.shape[0] - 1:
            ones_column = np.ones((X.shape[0], 1))
            X = np.concatenate((ones_column, X), axis=1)
